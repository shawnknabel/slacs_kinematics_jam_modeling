...
#                    = calculate_mass_and_slope_enclosed(surf_pot, sigma_pot, qobs_pot, self.distance, radius=self.theta_E, plot=True)
    ...
    def summary_plot(self, burn=0, save=False):
        ...
        if self.anisotropy == 'const':
            plot_pars[:,2] = self.anisotropy_ratio_accepted.astype(float)
        ...
        plt.clf()
        corner_plot(plot_pars[finite], plot_probs[finite], labels=self.labels, extents=plot_bounds, truths=plot_truths, truth_color='k', fignum=1)
