...
def log_rebin(lam_range, spec, oversample=1, velscale=None, flux=False):
    """
    ...
    :param lam_range: two elements vector containing the central wavelength
        ...
        standard FITS keywords: lam_range = CRVAL1 + [0, CDELT1*(NAXIS1 - 1)].
        It must be lam_range[0] < lam_range[1].
    :param spec: input spectrum or multiple spectra to rebin logaritmically.
    ...
    :param flux: (boolean) True to preserve total flux. In this case the
        ...
        beween the spectral shape before and after LOG_REBIN::
           ...
           plt.plot(np.linspace(lam_range[0], lam_range[1], spec.size), spec)
    ...
    assert len(lam_range) == 2, 'lam_range must contain two elements'
    assert lam_range[0] < lam_range[1], 'It must be lam_range[0] < lam_range[1]'
    assert spec.ndim == 1 or spec.ndim == 2, 'input spectrum must be a vector or 2d array'
    n = spec.shape[0]
    m = int(n*oversample)

    dlam = np.diff(lam_range)/(n - 1)         # Assume constant dlam
    ...
    if velscale is None:                     # Velocity scale is set by user
        ...
        ln_lim[1] = ln_lim[0] + m*ln_scale

    newBorders = np.exp(np.linspace(*ln_lim, num=m+1)) # Logarithmically
    k = (newBorders - lim[0]).clip(0, n-1).astype(int)

    specNew = np.add.reduceat(spec, k)[:-1]     # Do analytic integral of step function
    specNew.T[...] *= np.diff(k) > 0            # fix for design flaw of reduceat()
    specNew.T[...] += np.diff(spec[k].T*(newBorders - borders[k]))  # Add to 1st dimension

    if not flux:
        specNew.T[...] /= np.diff(newBorders)   # Divide 1st dimension

    # Output np.log(wavelength): natural log of geometric mean
    ln_lam = np.log(np.sqrt(newBorders[1:]*newBorders[:-1])*dlam)
...
def determine_goodpixels(ln_lam, lam_range_temp, z, width=800):
    ...
    :param lam_range_temp: Two elements vectors [lam_min2, lam_max2] with the minimum
