...
def stellar_type(libary_dir_xshooter, dir_temperture, pp_weights_2700, bins,
    ...
    df = pd.DataFrame(pd.read_excel(dir_temperture, engine='openpyxl'))
    info=df.to_numpy()
    col = 1
...
def get_velocity_dispersion_deredshift(degree,
    ...
    measurements = np.zeros(shape=(N,5))
    for i in range(N):
        ...
        measurements[i, 0] = pp.sol[0] # velocity
        measurements[i, 1] = pp.sol[1] # VD
        measurements[i, 2] = pp.error*np.sqrt(pp.chi2)[0] # dV
        measurements[i, 3] = pp.error*np.sqrt(pp.chi2)[1] # dVD
        measurements[i, 4] = pp.chi2
                                            
    if VD_name==None:
        np.savetxt(save_dir + 'kinematics_systematics.txt', measurements, fmt='%1.4e')
    else:
        np.savetxt(save_dir + 'VD_%s.txt' % VD_name, measurements, fmt='%1.4e')
    
    return measurements # measurements is N rows with five columns



def kinematics_map(dir, save_dir, name,radius_in_pixels):
    '''
    ...
    measurements=np.loadtxt(save_dir + 'VD.txt')
    # Vel, sigma, dv, dsigma
    output=np.loadtxt(dir +'voronoi_2d_binning_' + name + '_output.txt')
