...
def get_ref_fitting(argv, Number_of_files, pixel_size):
    ...
    for i in range(Number_of_files-1):
        ...
        residuals_show(res.x, data0, datai)
...
def get_ref_quasar(argv, Number_of_files, pixel_size):
    '''
    we use the gaussian profile to fit the brightest quasar position and use that
    to determine the relative positions of each frame
    :param argv: arguments set up at the beginning
    ...
    reference = np.array([])
    for i in range(Number_of_files):
        datai = fits.getdata(directory+argv[i+2])
        datai = grouped_Avg(datai, datai.shape[2])[:, :, -1]
        max_indexi_1 = get_po(directory+"config", argv[i+2])[0]
        max_indexi_2 = get_po(directory+"config", argv[i+2])[1]
        max_indexi = (max_indexi_2 - 1, max_indexi_1 - 1)
        max_valuei = datai[int(max_indexi[0]),int(max_indexi[1])]
        initial_FWHM = 2.06 ## the FWHM of KCWI data in pixel size (0.7/0.3395)
        initial_guess = np.array([max_indexi[1], max_indexi[0], max_valuei,
                                  initial_FWHM])
        res = optimize.minimize(fitting_gaussian, initial_guess, method="Nelder-Mead", args=(datai), tol=1e-6)
        fitting_gaussian_forcheck(res.x, datai)
        reference = np.append(reference, -1*res.x[:2])
    reference = reference.reshape(Number_of_files, 2)
    reference[:, 0] = reference[:, 0] - np.amin(reference[:, 0])
    reference[:, 1] = reference[:, 1] - np.amin(reference[:, 1])
    reference = reference * pixel_size
    top_right_x = np.amax(reference[:, 0])
    top_right_y = np.amax(reference[:, 1])
    top_right_x = top_right_x + datai.shape[1] * pixel_size
    top_right_y = top_right_y + datai.shape[0] * pixel_size
    print(reference)
    return reference, top_right_x, top_right_y

def get_ref_sersic(argv, Number_of_files, pixel_size):
    '''
    we use the gaussian profile to fit the brightest quasar position and use that
    to determine the relative positions of each frame
    :param argv: arguments set up at the beginning
    ...
    for i in range(Number_of_files):
        ...
        max_indexi = (max_indexi_2 - 1, max_indexi_1 - 1)
        max_valuei = datai[int(max_indexi[0]), int(max_indexi[1])]
        r_eff, n, ellip, theta = 2, 4, 0, 0 # initial guesses
        initial_guess = np.array([max_indexi[1], max_indexi[0], max_valuei,
                                  r_eff, n, ellip, theta])
        res = optimize.minimize(fitting_sersic, initial_guess, method="Nelder-Mead", args=(datai), tol=1e-6)
        fitting_sersic_forcheck(res.x, datai)
        reference = np.append(reference, -1*res.x[:2])
    reference = reference.reshape(Number_of_files, 2)
    reference[:, 0] = reference[:, 0] - np.amin(reference[:, 0])
    reference[:, 1] = reference[:, 1] - np.amin(reference[:, 1])
...
def fitting_gaussian(x0, data):
    x_0, y_0, amplitude, sigma = x0
    if amplitude < 0 or sigma < 0:
        chi2 = np.inf
    else:
        x, y = np.meshgrid(np.arange(data.shape[1]), np.arange(
            data.shape[0]))
        mod = Gaussian2D(amplitude=amplitude, x_mean=x_0, y_mean=y_0,
                         x_stddev=sigma,
                         y_stddev=sigma)
        img = mod(x, y)
        chi2 = np.sum(((data - img)**2))
    return chi2

def fitting_gaussian_forcheck(x0, data):
    x_0, y_0, amplitude, sigma = x0
    if amplitude < 0 or sigma < 0:
        print('amplitude or sigma is < 0')
    else:
        x, y = np.meshgrid(np.arange(data.shape[1]), np.arange(
            data.shape[0]))
        mod = Gaussian2D(amplitude=amplitude, x_mean=x_0, y_mean=y_0,
                         ...
                         y_stddev=sigma)
        img = mod(x, y)
        plt.imshow(data - img, origin='lower')
        plt.show()


def fitting_sersic(x0, data):
    x_0, y_0, amplitude, r_eff, n, ellip, theta = x0
    if amplitude<0 or r_eff <0 or n<0.3:
        chi2 = np.inf
    else:
        x, y = np.meshgrid(np.arange(data.shape[1]), np.arange(
            data.shape[2]))
...
def fitting_sersic_forcheck(x0, data):
...
def residuals_show(w, data0, datai):
    '''
    show the residuals when finding the relative positions
    :param w:
    :param data0:
    :param datai:
    :return:
    ...
    diff_x, diff_y, ratio = w
    data0_new = ndimage.shift(data0, [diff_y, diff_x])
    plt.imshow(data0_new*ratio-datai, origin="lower")
    plt.show()
