...
from drizzle3D_modules import get_ref_fitting, get_ref_quasar, get_ref_sersic
...
if find_position == 2:
    ...
    print('Using the chi2 of the frame i and j to determine the relative '
          'positions of each frame. Warning: possibly not accurate enough')
    start = time.time()
...
elif find_position == 4:
    F_re_x_min, F_re_y_min = 0, 0
    print('Using brightest quasar to determine the relative positions of each '
          'frame.')
    start = time.time()
    reference_po, F_re_x_max, F_re_y_max = get_ref_quasar(input_file, Number_of_files, d_orig)
    end = time.time()
    print("time for finding the reference positions:", round(end - start, 2), "seconds")
    final_image, final_cover = combine_frames_findpo(input_file, d_orig, d_drizzle, d_fine, reference_po, F_re_y_min,
                                                      F_re_y_max, F_re_x_min, F_re_x_max)

elif find_position == 5:
    F_re_x_min, F_re_y_min = 0, 0
    print('Using lensing galaxy light to determine the relative positions of '
          'each frame.')
    start = time.time()
    reference_po, F_re_x_max, F_re_y_max = get_ref_sersic(input_file, Number_of_files, d_orig)
    end = time.time()
    print("time for finding the reference positions:", round(end - start, 2), "seconds")
    final_image, final_cover = combine_frames_findpo(input_file, d_orig, d_drizzle, d_fine, reference_po, F_re_y_min,
                                                      F_re_y_max, F_re_x_min, F_re_x_max)
    ...
    raise ValueError('dither pattern (default) = 1, wcs = 2, entire frame = '
                     '3, quasar = 4, lensing galaxy = 5')
endtotal = time.time()

hdul= header(final_image, d_fine, F_re_y_min, F_re_x_min, input_file)
hdul.writeto(directory + "drizzled_cube.fits", overwrite=True)
...
fits.writeto(directory + "drizzled_cover.fits", final_cover, overwrite=True)
