...
def getMaskInFitsFromDS9reg(input,nx,hdu):
    ...
    mask = r.get_mask(shape=(nx, nx),hdu)
    return mask
...
def get_voronoi_binning_data(mos_dir, kin_dir, mos_name, obj_name):
...
def get_velocity_dispersion_deredshift(degree,
    ...
    for i in range(N):
        ...
        measurements = np.vstack((measurements, np.concatenate((pp.sol[:2],
            ...
            pp.chi2))[:2]))))
    if VD_name==None:
        np.savetxt(dir + name + 'VD.txt', measurements, fmt='%1.4e')
    else:
        np.savetxt(dir + name + 'VD_%s.txt' % VD_name, measurements, fmt='%1.4e')
...
def kinematics_map(dir, name, radius_in_pixels):
    ...
    measurements=np.loadtxt(dir + name + 'VD.txt')
    # Vel, sigma, dv, dsigma
...
def kinematics_map_systematics(dir, name, radius_in_pixels):
    '''
    ...
    measurements=np.loadtxt(f'{dir}/{name}_final_kinematics/{name}_velocity_dispersions_binned.txt')
    ...
    for i in range(output.shape[0]):
        num=int(output.T[2][i])
        VD = measurements[num][1]
        dVD = measurements[num][3]
        #v = measurements[num][0]
        #dv = measurements[num][2]

    final=np.vstack((output.T, VD, dVD))#, V_array, dv_array))
