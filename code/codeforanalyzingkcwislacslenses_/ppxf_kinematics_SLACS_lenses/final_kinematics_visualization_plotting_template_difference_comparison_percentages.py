...
for obj_name in obj_names:
    ...
    VD_2d = (VD_2d_2 - VD_2d_1)/VD_2d_1*100
    dVD_2d = (dVD_2d_2 - dVD_2d_1)/VD_2d_1*100
    V_2d = (V_2d_2 - V_2d_1)/VD_2d_1*100
    dV_2d = (dV_2d_2 - dV_2d_1)/VD_2d_1*100
    
    VD_2d_max = np.nanmax(np.abs(VD_2d))
    dVD_2d_max = np.nanmax(np.abs(dVD_2d))
    V_2d_max = np.nanmax(np.abs(V_2d))
    dV_2d_max = np.nanmax(np.abs(dV_2d))
    
    print('Maximum percentage differences')
    print(VD_2d_max, dVD_2d_max, V_2d_max, dV_2d_max)
    
    VD_2d_mean = np.nanmean(np.abs(VD_2d))
    dVD_2d_mean = np.nanmean(np.abs(dVD_2d))
    V_2d_mean = np.nanmean(np.abs(V_2d))
    dV_2d_mean = np.nanmean(np.abs(dV_2d))
    
    print('Maximum percentage differences')
    print(VD_2d_mean, dVD_2d_mean, V_2d_mean, dV_2d_mean)
    ...
    cbar1 = plt.colorbar()
    cbar1.set_label(r'Percent Difference')
    plt.title(f"{obj_name} velocity dispersion difference")
    plt.xticks(ticks_pix, labels=ticklabels)
    plt.yticks(ticks_pix, labels=ticklabels)
    plt.xlabel('arcsec')
    plt.ylabel('arcsec')
    plt.savefig(save_dir_2 + obj_name + '_VD_difference_percentage_map.png')
    plt.savefig(save_dir_2 + obj_name + '_VD_difference_percentage_map.pdf')
    plt.pause(1)
    plt.clf()
    ...
    cbar2 = plt.colorbar()
    cbar2.set_label(r'Percent Difference')
    plt.title(f"{obj_name} velocity dispersion error difference")
    plt.xticks(ticks_pix, labels=ticklabels)
    plt.yticks(ticks_pix, labels=ticklabels)
    plt.xlabel('arcsec')
    plt.ylabel('arcsec')
    plt.savefig(save_dir_2 + obj_name + '_dVD_difference_percentage_map.png')
    plt.savefig(save_dir_2 + obj_name + '_dVD_difference_percentage_map.pdf')
    ...
    cbar3 = plt.colorbar()
    cbar3.set_label(r'Percent Difference')
    plt.title(f"{obj_name} velocity difference")
    plt.xticks(ticks_pix, labels=ticklabels)
    plt.yticks(ticks_pix, labels=ticklabels)
    plt.xlabel('arcsec')
    plt.ylabel('arcsec')
    plt.savefig(save_dir_2 + obj_name + '_V_difference_percentage_map.png')
    plt.savefig(save_dir_2 + obj_name + '_V_difference_percentage_map.pdf')
    plt.pause(1)
    plt.clf()
    ...
    cbar2 = plt.colorbar()
    cbar2.set_label(r'Percent Difference')
    plt.title(f"{obj_name} velocity error difference")
    plt.xticks(ticks_pix, labels=ticklabels)
    plt.yticks(ticks_pix, labels=ticklabels)
    plt.xlabel('arcsec')
    plt.ylabel('arcsec')
    plt.savefig(save_dir_2 + obj_name + '_dV_difference_percentage_map.png')
    plt.savefig(save_dir_2 + obj_name + '_dV_difference_percentage_map.pdf')
    plt.pause(1)
    plt.clf()
