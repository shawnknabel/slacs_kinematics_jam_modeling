...
07/12/22 - only kinematics, R=1
07/13/22 - adjusting for contamination by source - see email 'J0330 - the ...'
'''
...
background_source_spectrum = f'{dir}{obj_abbr}_spectrum_background_source.fits'
...
wave_min = 320
wave_max = 410 #428 - adjusted for contamination from source
velscale_ratio = 2
...
'''
Step 2.5: remove background source
'''
# check it first
...
hdu_nobs.writeto(f'{dir}{obj_name}_nobs_crop.fits', overwrite=True) # "_crop" because it get pulled into the voronoi binning data for actual kinematics fitting by CF's function "get_voronoi_binning_data"

# set name for cropped background-source-removed mosaic
name_nobs = f'{obj_name}_nobs'

hdu_noise = hdu.copy()
hdu_noise[0].data = noise_cube
hdu_nobs.writeto(f'{dir}{obj_name}_noise.fits', overwrite=True)

hdu_bs = hdu.copy()
hdu_bs[0].data = background_source_cube
hdu_nobs.writeto(f'{dir}{obj_name}_bs.fits', overwrite=True)
...
region = dir + 'background_source.reg'
...
fits.writeto(dir + name + '_SN_per_AA_residuals.fits', residuals, overwrite=True) # uses nobs data

'''
refine the mask --- Don't really need do this I think?
'''
...
SN_y_center, SN_x_center = unravel_index(SN_per_AA_model.argmax(), SN_per_AA_model.shape) # uses nobs data
max_radius = 50
target_SN = 1.

origin_imaging_data_perAA = np.mean(hdu[0].data[ind_min_SN:ind_max_SN,:,:], # not saved, no worries
                                 axis=0)*2 

# select the region for binning
plt.figure()
select_region(dir, origin_imaging_data_perAA, SN_per_AA_model,
              SN_x_center,SN_y_center, radius_in_pixels,
              max_radius,
              target_SN, name) # this "name" is fine as is because the SN_perAA_model, etc is all with the nobs mosaic
...
plt.figure()
voronoi_binning(20, dir, name) # this does the binning map from nobs data, doesn't need nobs in the function
plt.tight_layout()
plt.savefig(dir + obj_name + '_voronoi_binning.png') 
plt.pause(1)
plt.clf()

## get voronoi_binning_data based on the map, creates the 'voronoi_binning_' + name + '_data.fits' file
get_voronoi_binning_data(dir, name) # kcwi_util adjusted to take nobs_crop if it exists
