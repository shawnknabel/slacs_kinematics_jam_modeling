...
08/22/22 - adjusting for contamination by source - see email 'J0330 - the ...'
08/25/22 - adjustments worked for initial kinematics! Now let's see what happens here.
...
# wavelength range
wave_mins = [315,320,325] # wider, initial, narrower
wave_maxs = [415,410,405] 
...
#KCWI mosaic datacube
name = f'KCWI_{obj_abbr}_icubes_mosaic_0.1457'
# Take the background-source subtracted cropped data! It does, no worries :)

# speaking of background sources...
    ...
    for a in range(len(apertures)): # 2 aperture sizes
        ...
        for d in range(len(degrees)):
            ...
            for w in range(len(wave_mins)):
                ...
                # get the global spectrum
                templates, pp, lamRange1, logLam1, lamRange2, logLam2, galaxy, background_source = \
    ppxf_kinematics_RXJ1131_getGlobal_lens_deredshift(library,
                                                      degree=degree,
