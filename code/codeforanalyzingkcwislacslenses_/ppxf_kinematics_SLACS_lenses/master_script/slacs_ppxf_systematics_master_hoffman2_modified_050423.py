...
04/27/23 - This has been further modified following tests of systematics that showed the wavelength ranges and degrees should be adjusted a little.
- Wavelength ranges for the no-G set are still in consideration... The continuum starts to turn up at the longer end of the largest range around 4200 A. I need to ask Tommaso.
- Degrees should be deg = range / 250 A
    low ends = [0.3225, 0.3250, 0.3275]
    no G hi = [0.4025, 0.4050, 0.4075]
    G hi = [0.4325, 0.4350, 0.4375]
So no-G is 750, 800, 850 A range (3/4)
And G is 1050, 1100, and 1150 A range (4/5)
...
kinematics_dir = f'{data_dir}{date_of_kin}/'#f'{kinematics_full_dir}{date_of_kin}/'
# create a systematics directory
systematics_dir = f'{data_dir}hoffman2_systematics_{today}/'
...
# MODIFICATION 04/27/23
wave_mins = np.array([322.5, 325.0, 327.5])
wave_maxs = np.array([402.5, 405.0, 407.5])
# for G-band add 300 A or 30 nm

# degree of the additive Legendre polynomial in ppxf
# MODIFICATION 04/27/23
# each 
degrees = [3,4,5] # 90, 100, 110, 120, 130 should have different
...
for i, obj_name in enumerate(obj_names.to_numpy()[:,0]):
    ...
    ####### 05/04/23 - This is what I fucked up...
                ...
                for a in range(len(apertures)): # 2 aperture sizes
                    ...
                    for d in range(len(degrees)):
                        ...
                        for w in range(len(wave_mins)):
                            ...
                            # if g, add 30 nm
                            if g_band == 'g_band':
                                wave_max = wave_max+30
                                
                            ###############
                            # This is modification of degree with wavelength range
                            # degree 3 or 4
                            if (wave_max-wave_min <= 100) & (degree == 5):
                                continue
                            # degree 4 or 5
                            if (wave_max-wave_min > 100) & (degree == 3):
                                continue
