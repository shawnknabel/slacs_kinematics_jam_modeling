...
###########################################################        
    ...
    def datacube_visualization(self):
        ...
        data_hdu = fits.open(self.kcwi_datacube)
    ...
    def ppxf_central_spectrum(self):
        ...
        # create a noise
        noise_array = np.full_like(galaxy, noise) # Assume constant noise per
        ...
        pp = ppxf(self.templates, 
                  ...
                  sky=self.background_source, 
                  lam=self.rest_wave,
                  lam_temp=self.wave_range_templates,
                 )
        ...
        self.global_template = pp.templates @ pp.weights[:nTemplates]
    ...
    def crop_datacube(self):
        ...
        datacube=data_hdu[0].data
        data_hdu.close()
        self.cropped_datacube = datacube[:, lens_center_y - r-1:lens_center_y +
                                                              r, lens_center_x
                                                                   - r -1:
                                                                 lens_center_x + r]
    ...
    def create_SN_map(self):
        ...
        noise_from_blank = self.cropped_datcube[3000:4000, 4-3:4+2,4-3:4+2]
        ...
        # find indices for SN
        ind_min_SN = find_nearest(lin_axis_sky*(1+self.z), 4800)
        ind_max_SN = find_nearest(lin_axis_sky*(1+self.z), 5100)
        ...
        sigma_per_half_pixel = np.std(self.noise_cube[ind_min_SN:ind_max_SN,:,:], axis=0)
