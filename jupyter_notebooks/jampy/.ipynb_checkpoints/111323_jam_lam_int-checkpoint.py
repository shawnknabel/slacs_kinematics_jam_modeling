...
def jam_bestfit (pars, **kwargs):
    ...
    if geometry=='axi':
        ...
        surf_pot, sigma_pot, qobs_pot, lambda_internal = total_mass_mge(surf_lum, sigma_lum, qobs_lum, qobs_eff,
...
def summary_plot(obj_name, date_time, model_dir, jam_prob_func, model_name,                      pars=None, lnprob=None, labels=None, bounds=None, lensprior=None,                     kwargs=None, save=False, load=False):
    ...
    for i, par in enumerate(pars):
        ...
        zlens = kwargs['zlens']
        zsource = kwargs['zsource']
        lambda_internal = power_law_mge (gamma, theta_E, q, rbreak, reff, lambda_int, rs_mst, zlens, zsource, plot=False, return_lambda_int=True)
...
def total_mass_mge (surf_lum, sigma_lum, qobs_lum, qobs_eff,
    ...
    if model == 'nfw':
        ...
        surf_pot, sigma_pot, qobs_pot, lambda_internal = power_law_mge(gamma, theta_E, qobs_eff, rbreak, reff, lambda_int, rs_mst, zlens, zsource, plot)
        #plt.pause(1)
...
def power_law_mge (gamma, theta_E, q, rbreak, reff, lambda_int, rs_mst, zlens, zsource, plot=False, return_lambda_int=False):
    """
    ...
    kappa_mst, lambda_internal = mass_sheet_transform(kappa, lambda_int, rbreak, reff, rs_mst, zlens, zsource)
...
def mass_sheet_transform (kappa, lambda_int, rbreak, reff, rs_mst, zlens, zsource):
    ...
    for test in lambda_ints:
        ##### This line is a mistake, lambda_int should be "test" instead, hashed so it won't run #kappa_bounds = kappa * lambda_int + (1 - test) * kappa_s
...
# # 11/13/23 - I want some kind of function to determine the maximum lambda_int for a given model... I'm only really able to determine *where* it will be negative, not the lambda_int that maximizes it.

# In[14]:


def mst (r, gamma, theta_E, lambda_int, rscale):
    
    pure_power_law = (3 - gamma)/2 * (theta_E/r)**(gamma-1)
    mass_sheet_trunc = rscale**2/(rscale**2+r**2)
    kappa_mst = pure_power_law * lambda_int + (1 - lambda_int)*mass_sheet_trunc
...
#mst(r, gamma, theta_E, lambda_int, rscale)

f = lambda lambda_int: mst(r, gamma, theta_E, lambda_int, rscale)
...
kappa = mst (r, gamma, theta_E, lambda_int, rscale)
print(any(kappa<0))
#plt.plot(r[kappa<0], kappa[kappa<0])
#plt.ylim(-0.1, 0.1)
plt.loglog(r,kappa)
plt.show()


# # What if I just try lambda_ints until they receive a zero?

# In[17]:


lambda_ints = np.linspace(1.0,1.2,1000)
reff = 2.180866363995832
rscales = reff*np.linspace(5,10,10)
gammas=np.linspace(2.0, 2.5, 10)
...
breaks = np.zeros((10,10))

for i, gamma in enumerate(gammas):
    for j, rscale in enumerate(rscales):
        print('rscale:',rscale)
        for lambda_int in lambda_ints:
            kappa = mst(r, gamma, theta_E, lambda_int, rscale)
            if any(kappa<0):
                print('lambda_int gives negative at ', lambda_int)
                breaks[i,j] = lambda_int
                break
...
for i in range(len(breaks)):
    ax.plot(rscales, breaks[i], label="{:.2f}".format(gammas[i]))
ax.legend()
ax.set_title('J0029')
ax.set_ylabel(r'$\lambda_{int, max}$', fontsize=16)
ax.set_xlabel(r'$r_{scale}$ [arcsec]', fontsize=16)
...
for i, gamma in enumerate(gammas):
    for j, rscale in enumerate(rscales):
        ...
        for lambda_int in lambda_ints:
            ...
            if any(kappa<0):
                print('lambda_int gives negative at ', lambda_int)
                print('This happens at a radius of ', np.min(r[kappa<0]))
                breaks[i,j] = lambda_int
                break
...
# # 10/31/23 - Running two objects with only priors.
                ...
                for w, geometry in enumerate([geoms[0]]):
                    ...
                    for m, align in enumerate([aligns[0]]):
                        ...
                        for n, lensprior in enumerate([lenspriors[0]]):
                            ...
                            space_jam (obj_name, SN, model, anisotropy, geometry, align,                                         sampler='emcee', sampler_args=sampler_args,                                        rs_mst=None,                                        p0=p0, bounds=bounds, sigpar=sigpar, prior_type=prior_type, lensprior=lensprior,                                        date_time=date_time, overwrite=True, run_id=3, test_prior=False)#True)
...
np.rad2deg(0.8645314638790208)


# In[ ]:


inc = 0.8645314638790208
np.sqrt(np.cos(inc)**2 + (0.05*np.sin(inc))**2)


# In[ ]:


np.sqrt(0.65**2 - np.cos(inc)**2)/np.sin(inc)**2
