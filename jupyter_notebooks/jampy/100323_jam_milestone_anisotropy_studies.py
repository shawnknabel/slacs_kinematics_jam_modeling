...
# # 10/02/23 - Copied from home/shawnknabel/Documents/slacs_kinematics/jupyter_notebooks/jampy/100223_jam_milestone_anisotropy_studies.ipynb
...
lens_models=lens_models_chinyi
...
# # Checking the MGEs to see if any qobs are less than 0.6 (at least for the slow rotators)

# In[9]:


plt.rcParams.update({'font.size': 14})
qobs_array = np.empty(14, dtype=object)
surfs_array = np.empty_like(qobs_array)
sigs_array = np.empty_like(qobs_array)
...
for i, obj_name in enumerate(obj_names):
    ...
    fast_slow = paper_table[paper_table['obj_name']==obj_name]['class_for_JAM_models'].to_numpy()[0]
    ...
    axs[0].scatter(np.ones_like(qobs)*i, qobs, marker=marker, color=color)
    axs[0].axhline(0.6)
    axs[1].plot(np.log10(surf[surf.argsort()[::-1]]), qobs[surf.argsort()[::-1]], marker=marker)
    axs[1].axhline(0.6)
    axs[2].plot(sigma[sigma.argsort()[::-1]], qobs[sigma.argsort()[::-1]], marker=marker)
    axs[2].axhline(0.6)
    
    print(obj_name)
    print('number of MGE components: ', len(qobs))
    
axs[0].tick_params(labelrotation=90, )
axs[0].set_xticks(range(14))
axs[0].set_xticklabels(obj_names)
axs[0].set_ylabel('MGE axis ratios')
axs[1].set_xlabel(r'MGE $log_{10}(peak)$')
axs[2].set_xlabel(r'MGE $\sigma$')


# # What I will do for now is ignore the fast rotators with q_min < 0.6 (i.e. SDSSJ1531-0105 so far)
...
def get_priors_and_labels (model, anisotropy, qobs, geometry, align, fast_slow, p0=None, bounds=None, sigpar=None, prior_type=None):
    ...
    if geometry=='axi':
        ...
        bounds[1][1] = qbound  
        label1 =  r"$q_{\rm min}$"
        # for slow rotators, update bounds so that q_min_intr > 0.6
        if fast_slow == 'slow':
            # bound it lower by q_intr_min > 0.6
            bounds[0][1] = 0.6
...
# # 10/03/23 - Running 8 objects with 3 models.
