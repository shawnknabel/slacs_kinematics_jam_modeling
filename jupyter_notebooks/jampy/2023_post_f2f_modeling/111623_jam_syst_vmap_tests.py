...
# # 11/16/23 - Models with percent level deviations in velocity map for systematic tests
# ## Copied from home/shawnknabel/Documents/slacs_kinematics/jupyter_notebooks/jampy/111623_jam_const_err.ipynb
# ## Adding a keyword to "space_jam" which will allow small variations in Vrms
# ____
# #### 11/16/23 - Running models with constant error of 5% instead of the error I measured
# ##### Copied from home/shawnknabel/Documents/slacs_kinematics/jupyter_notebooks/jampy/111523_jam_lam_int.ipynb
# ____
# #### 11/15 - copied from 11/14, gotta work fast
# _____
...
def create_model_directory (target_jam_dir, obj_name, SN, 
    ...
    model_name = f'{SN}_{model}_{anisotropy}_{geometry}_{align}_{lensprior}_{rs_mst}_{constant_err}_{kinmap_test}'
...
def space_jam (obj_name, SN, model, anisotropy, geometry, align, 
    ...
    if kinmap_test is not None:
        ...
        Vrms = kinmap_test * Vrms
        dVrms = kinmap_test * dVrms
...
# run at 5% constant error
constant_err=False
# test scaling kinematics and errors
kinmap_tests = [0.98, 0.99, 1.01, 1.02]
            ...
            for l, anisotropy in enumerate([anis[0]]):
                ...
                for w, geometry in enumerate(geoms):
                    ...
                    for m, align in enumerate(aligns):
                        ...
                        for n, lensprior in enumerate(lenspriors):
                            
                            for p, kinmap_test in enumerate(kinmap_tests):
                                
                                print(f'Commencing JAM routine with model {model}, anisotropy {anisotropy}, geometry {geometry}, and alignment {align}... {lensprior}...{kinmap_test}... number of steps {nstep}')
                                ...
                                space_jam (obj_name, SN, model, anisotropy, geometry, align,                                             sampler='emcee', sampler_args=sampler_args,                                            rs_mst=None,                                            p0=p0, bounds=bounds, sigpar=sigpar, prior_type=prior_type, lensprior=lensprior,                                            date_time=date_time, overwrite=True, run_id=4, 
                                           test_prior=False, constant_err=constant_err, kinmap_test=kinmap_test)#True)
