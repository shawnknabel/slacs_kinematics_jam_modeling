...
# 12/04/23 - Normalizing mass profile by dividing by q
...
def power_law_mge (gamma, theta_E, q, rbreak, reff, k_mst, rs_mst, zlens, zsource, plot=False, return_lambda_int=False):
    ...
    m = mge_fit_1d(r, surf_mass_dens, ngauss=30, inner_slope=2, outer_slope=3, quiet=1, plot=plot) # this creates a circular gaussian with sigma=sigma_x (i.e. along the major axis)
    ...
    # 11/21/23 - I might be normalizing incorrectly here... I think I need to divide by (2 * np.pi * sigma_pot**2 * q)
    surf_pot = surf_pot_tot / (2 * np.pi * sigma_pot**2 * q)
...
def space_jam (obj_name, SN, model, anisotropy, geometry, align, 
               ...
               date_time=None, overwrite=False, run_id=None, test_prior=False, constant_err=False):
                ...
                for w, geometry in enumerate(geoms):
                    ...
                    for m, align in enumerate(aligns):
                        ...
                        for n, lensprior in enumerate(lenspriors):
                            ...
                            space_jam (obj_name, SN, model, anisotropy, geometry, align,                                         sampler='emcee', sampler_args=sampler_args,                                        rs_mst=None,                                        p0=p0, bounds=bounds, sigpar=sigpar, prior_type=prior_type, lensprior=lensprior,                                        date_time=date_time, overwrite=True, run_id=2, test_prior=False, constant_err=constant_err)#True)
